...
const status = ref("Starting VistaWASM...");
const stats = ref("");
...
function input(id: string): HTMLInputElement {
  const element = panel.value?.querySelector<HTMLInputElement>(`#${id}`);
...
function select(id: string): HTMLSelectElement {
  const element = panel.value?.querySelector<HTMLSelectElement>(`#${id}`);
...
function buildFractalOptions(): FractalTerrainOptions {
  ...
  return {
    seed: readNumber(input("seed"), 2222),
...
function drawMinimapMarker(position: [number, number, number], yawRadians: number): void {
  if (!minimapBaseImage || !latestMetadata || !minimap.value) {
    return;
  }
...
async function refreshExportData(): Promise<void> {
  if (!engine || !latestMetadata || !minimap.value) {
    return;
  }

  latestHeights = engine.exportHeightmap();
  renderHeightmapToCanvas(minimap.value, latestMetadata, latestHeights);
  const context = minimap.value.getContext("2d");
...
async function generate(): Promise<void> {
  ...
  status.value = "Generating terrain...";
  ...
  try {
    ...
    status.value = `Seed ${options.seed} ready (${handle.metadata.width}x${handle.metadata.height}).`;
  } catch (error) {
    showError(error);
  } finally {
...
onMounted(async () => {
  ...
  try {
    ...
    engine = await createVistaEngine(element, {
      render: {
    ...
    flyControls = attachFlyCameraControls(engine, element, {
...
<template>
  <main class="shell">
    <section class="viewer">
      <canvas ref="canvas" class="vista-canvas" aria-label="VistaWASM terrain view" />
      <canvas ref="minimap" class="minimap" aria-label="Terrain minimap" title="Top-down terrain map" />
    ...
    <aside ref="panel" class="controls" aria-label="VistaWASM controls">
      <fieldset>
        <legend>Terrain</legend>
        <label>
          Seed
          <input id="seed" type="number" value="2222" step="1">
        </label>
        <label>
          Size
          <select id="size">
      ...
      <fieldset>
        ...
        <button type="button" :disabled="generating" @click="generate().catch(showError)">
          Generate terrain
        </button>
        <button type="button" @click="exportScreenshot">Save screenshot (PNG)</button>
        <button type="button" @click="exportMap().catch(showError)">Download terrain map (PNG)</button>
        <button type="button" @click="exportModel">Download 3D model (OBJ)</button>
        <button type="button" @click="exportHeightmapBytes">Download raw heightmap</button>
      </fieldset>
    </aside>
  </main>
</template>
