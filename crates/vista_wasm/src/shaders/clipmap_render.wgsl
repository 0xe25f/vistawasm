...
// --- Bank strips ---------------------------------------------------------
//
// Strips beside streams narrower than a heightmap sample, drawn over the
...
struct BankIn {
  ...
  // x: 0 at the water to 1 at the outer edge, y: strip width, z: speed.
...
fn vertex_bank(in: BankIn) -> BankOut {
  ...
  let drawn = max(in.params.y, 0.75 * footprint);
  let position = in.position + vec3<f32>(in.outward.x, 0.0, in.outward.y) * (drawn - in.params.y) * in.params.x;
  out.clip_position = frame.view_proj * vec4<f32>(position, 1.0);
  out.world_position = position;
  out.outward = in.outward;
  out.edge = in.params.x;
  out.speed = in.params.z;
  out.coverage = in.params.y / drawn;
  return out;
}
